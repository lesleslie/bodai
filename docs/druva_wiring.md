# Druva Wiring Checklist (Ecosystem)

This document describes **what still needs to be wired** to make Druva useful
as the ecosystem’s persistent store, now that `put/get` and time‑series tools
exist in the Druva MCP server.

## Current Consumers (Already in Code)

### Mahavishnu

- **Health persistence** uses `put(key, value, ttl)` via `DruvaClient`.

  - File: `mahavishnu/core/health_integration.py`
  - Key format: `health:adapter:{adapter_name}` (TTL 7 days)
  - **Status:** Ready once Druva MCP server is running.
...
- **Adapter registry push** writes to Druva via `store_adapter`.
  - File: `oneiric/adapters/druva_pusher.py`
  - **Status:** Works when Druva MCP server is up.

## Missing Producers (Need Wiring)

### 1. Crackerjack → Druva Time‑Series
...
### 2. Optional: Mahavishnu → Druva Time‑Series

If Mahavishnu generates any cross‑repo workflow metrics, those can be
persisted to Druva too. Nothing currently writes from Mahavishnu.
...
Available tools on Druva MCP server:
...
1. **Mahavishnu**: no change needed for reads; it already queries Druva.
1. **Druva**: ensure MCP server is running on `http://localhost:8683/mcp`.
